...
import { Avatar, Card, Chip, LeaveTypeBadge, ScoreBar, StatusChip } from '@/components/ui'
...
export function ApproverHistoryClient({ profile, requests }: {
  profile: Profile
  ...
  const filtered = useMemo(() => {
    ...
    list.sort((a, b) => {
      ...
      if (sortKey === 'submitted_at') {
        ...
        va = a.ai_score ?? 0; vb = b.ai_score ?? 0
      }
    ...
    <AppShell profile={profile}>
      <div style={{ display: 'flex', height: '100%', overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          ...
          {/* Toolbar */}
          <div style={{
            padding: '16px 20px', borderBottom: `1px solid ${T.hairline}`,
            background: T.paper, display: 'flex', flexDirection: 'column', gap: 12,
          }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <div style={{ fontSize: 16, fontWeight: 600 }}>ประวัติคำขอลา</div>
              ...
              <div style={{
                ...
                padding: '7px 12px', background: T.surface, width: 220,
              }}>
                ...
                <input
                  ...
                  placeholder="ค้นหาชื่อ, แผนก, เหตุผล…"
                  style={{ border: 'none', outline: 'none', fontSize: 13, fontFamily: 'inherit', flex: 1, background: 'transparent', color: T.ink }}
                />
                {search && (
                  <button onClick={() => setSearch('')} style={{ border: 'none', background: 'none', cursor: 'pointer', color: T.inkFaint, padding: 0, fontSize: 14, lineHeight: 1 }}>✕</button>
                )}
              </div>
            </div>
          ...
          {/* Table header */}
          <div style={{
            display: 'grid', gridTemplateColumns: '2fr 1.2fr 1fr 80px 80px 72px',
            padding: '8px 20px', fontSize: 11, color: T.inkFaint,
            textTransform: 'uppercase', letterSpacing: '0.06em',
            borderBottom: `1px solid ${T.hairline}`, background: T.paper,
            gap: 12,
          }}>
            <span>พนักงาน</span>
            <span>ประเภท / วันที่</span>
            <button onClick={() => toggleSort('days')} style={thBtnStyle}>
              จำนวน <SortIcon k="days" />
            </button>
            <button onClick={() => toggleSort('ai_score')} style={thBtnStyle}>
              AI <SortIcon k="ai_score" />
            </button>
            <span>สถานะ</span>
            <button onClick={() => toggleSort('submitted_at')} style={thBtnStyle}>
              วันที่ยื่น <SortIcon k="submitted_at" />
            </button>
          </div>
          ...
          <div style={{ flex: 1, overflowY: 'auto' }}>
            {filtered.length === 0 && (
              <div style={{ textAlign: 'center', padding: '60px 20px', color: T.inkFaint, fontSize: 13 }}>
                ไม่พบรายการ
              </div>
            )}
            {filtered.map(req => (
              ...
              >
                ...
                <span style={{
                  ...
                  color: req.ai_score == null ? T.inkFaint
                    : req.ai_score >= 80 ? T.good
                    : req.ai_score >= 50 ? T.warn : T.bad,
                }}>
                  {req.ai_score ?? '—'}
                </span>
          ...
          {/* Footer stats */}
          <div style={{
            padding: '10px 20px', borderTop: `1px solid ${T.hairline}`,
            background: T.paper, display: 'flex', gap: 24, fontSize: 12, color: T.inkFaint,
          }}>
            ...
            <span style={{ marginLeft: 'auto' }}>วันลาอนุมัติรวม <strong style={{ color: T.ink }}>{stats.totalDays} วัน</strong></span>
          </div>
        </div>
        ...
        {selected && (
          <div style={{
            width: 320, borderLeft: `1px solid ${T.hairline}`,
            background: T.paper, overflowY: 'auto', padding: 20,
            ...
            {selected.ai_score !== null && (
              <Card style={{ padding: 14, background: T.accentTint, border: `1px solid ${T.accentSoft}`, display: 'flex', flexDirection: 'column', gap: 8 }}>
                ...
                {selected.ai_recommendation && (
                  <Chip kind="ai">แนะนำ: {selected.ai_recommendation}</Chip>
                )}
              </Card>
            )}
            ...
            {selected.approved_at && (
              <Card style={{ padding: 14, display: 'flex', flexDirection: 'column', gap: 6 }}>
                ...
                <div style={{ fontSize: 12, color: T.inkFaint }}>
                  {format(parseISO(selected.approved_at), 'd MMM yyyy HH:mm', { locale: th })}
                </div>
                {selected.approver_comment && (
                  <div style={{ fontSize: 13, color: T.inkSoft }}>{selected.approver_comment}</div>
                )}
              </Card>
            )}
          </div>
